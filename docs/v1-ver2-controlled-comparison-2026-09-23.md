...
**커밋 `506d013` 당시의 재점검에서는 아래 세 오류가 모두 미해결이었다.** 비교 기준 커밋 `b9c2bd4` 이후 당시 `ver2`의 제품 Python 파일과 `main.py`에 변경이 없음을 파일별 SHA-256으로 확인했다. 따라서 아래 표는 수정 전 상태를 기록한다. 기존 12개 사례를 같은 실행 도구로 다시 돌린 [재점검 원본](assessment/v1-ver2-controlled-2026-09-23/post-discovery-recheck/result.json)도 [최종 실행 원본](assessment/v1-ver2-controlled-2026-09-23/run-4/result.json)과 사례별 판정·제품 해시가 일치했다.

| 발견한 오류 | 코드에서 확인한 원인 | 이후 제품 코드 수정 | 재점검 결과 | 상태 |
|---|---|---|---|---|
| P06: 문자열 속 `@app.get('/sample')`을 라우트 변경으로 차단 | [변경 강도 분류](../drift_gate/core/classification/intensity.py)는 라우트처럼 보이는 텍스트를 탐지하고, [내용 검사](../drift_gate/core/evaluation/contracts.py)는 추가 줄의 라우트 패턴을 문자열 내부에서도 추출한다. | 없음 | 기대 `pass`, 실제 `fail` | **미해결 오탐** |
| P10: 문서 없는 HEAD 라우트 추가를 허용 | 내용 검사는 `head`를 인식하지만 [앞단 변경 강도 분류](../drift_gate/core/classification/intensity.py)의 라우트 패턴에는 `head`가 없어 규칙이 발동하지 않는다. | 없음 | 기대 `fail`, 실제 `pass` | **미해결 미탐** |
| B01: 올바른 Markdown 표의 API 변경을 차단 | [문서 내용 검사](../drift_gate/core/evaluation/contracts.py)가 `GET /members`처럼 연속된 메서드·경로를 찾으며 표 셀 `| GET | /members |`는 읽지 못한다. | 없음 | 기대 `pass`, 실제 `fail` | **미해결 형식 오탐** |

재점검에서는 제품 코드를 바꾸지 않았으므로 9/11 또는 0/1을 개선 후 수치로 갱신하지 않는다. 세 사례는 향후 수정을 검증할 **회귀 사례**로 남긴다. 실제 수정이 이루어지면 수정 커밋, 동일 사례의 전후 판정, 다른 정상·차단 사례의 회귀 결과를 별도로 기록해야 해결 상태로 바꿀 수 있다.

세 오류의 수정 위치·순서·검증 기준은 [E05 오류 수정 설계](e05-contract-drift-fix-design-2026-09-23.md)에 정리했다. 이 설계 문서는 수정 완료나 수정 후 성능을 뜻하지 않는다.

이후 제품 코드 `715335f`에서 세 사례를 수정했다. 기존 12개와 32개 반례의 전후 판정, 여전히 틀린 사례, 재현 가능한 원본은 [수정 후 재검증 보고서](v1-ver2-contract-fix-results-2026-09-23.md)에 기록했다. 위의 미해결 표와 9/11·0/1은 **수정 전 시점의 값**으로 남긴다.
